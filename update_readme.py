...
def update_readme(stats, repo_path="README.md"):
    ...
    for i, line in enumerate(readme):
        if "**Contest Rating**" in line:
            ...
            readme[i] = f"| 🟢 Easy        | {stats.get('easySolved', 0)} / {stats.get('totalEasy', 0)}          | {stats.get('acceptanceRate', '0')}%              | Beats {stats.get('acceptanceRate', '0')}%  |\n"
        elif "| 🟡 Medium" in line:
            readme[i] = f"| 🟡 Medium      | {stats.get('mediumSolved', 0)} / {stats.get('totalMedium', 0)}         | {stats.get('acceptanceRate', '0')}%             | Beats {stats.get('acceptanceRate', '0')}%  |\n"
        elif "| 🔴 Hard" in line:
            readme[i] = f"| 🔴 Hard        | {stats.get('hardSolved', 0)} / {stats.get('totalHard', 0)}            | {stats.get('acceptanceRate', '0')}%             | Beats {stats.get('acceptanceRate', '0')}%  |\n"
